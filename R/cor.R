...
amif <-
function(x, lag.max = NULL, maxbit = 20, confidence = 0.2, ci = 0.95,
         nsurr = 20, fft = FALSE, amplitude = FALSE, normalized = TRUE,
         trace = FALSE, plot = TRUE, ...)
{
    if(NCOL(x) > 1)
        stop("x is not a vector or univariate time series")
    if((maxbit < 1) | (maxbit > 25))
        stop("maxbit out of range")
    if((confidence < 0.01) | (confidence > 0.99))
        stop("confidence out of range")
    ...
    if(ci > 0) {
        ...
        for(i in 1:nsurr)
            surrwb[i,] <-
                .C("R_amif",
                   as.vector(surrsam[,i], mode="double"),
                   as.integer(sampleT),
                   inf=as.vector(inf),
                   as.integer(lag.max),
                   as.integer(maxbit),
    ...
    amif <- structure(.Data = list(acf = cor, type = "covariance",
                      n.used = sampleT, lag = lag,
                      series = series, snames = colnames(x), clim = wb,
...
plot.amif <-
function(x, ci.col = "blue", ylab = "AMIF", ...)
{
    if(!inherits(x, "amif"))
        stop("method is only for amif objects")
    plot.acf(x, ylab = ylab, ...)
    if(!is.null(x$clim))
        lines(x$lag[, 1, 1], x$clim, col = ci.col, lty = 2)
}
